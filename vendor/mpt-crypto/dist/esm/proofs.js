...
import { BLINDING_FACTOR_SIZE, CLAWBACK_PROOF_SIZE, CONVERT_BACK_PROOF_SIZE, CONVERT_PROOF_SIZE, CONTEXT_HASH_SIZE, ELGAMAL_TOTAL_SIZE, PEDERSEN_COMMIT_SIZE, PRIVKEY_SIZE, PUBKEY_SIZE, SEND_PROOF_SIZE, } from './constants.js';
import { bytesToHex, hexToBytes } from './hex.js';
import { assertUint64, rawParticipant, rawPedersenParams } from './internal.js';
import { withModule } from './runtime.js';
 ...
 */
export async function getConvertProof(publicKey, privateKey, contextHash) {
 ...
 * @throws If inputs are malformed or the WASM call fails.
 */
export async function getClawbackProof(privateKey, publicKey, contextHash, amount, ciphertext) {
 ...
 * @throws If inputs are malformed or the WASM call fails.
 */
export async function getConvertBackProof(privateKey, publicKey, contextHash, amount, params) {
    assertUint64(amount, 'amount');
    const priv = hexToBytes(privateKey, 'privateKey', PRIVKEY_SIZE);
    const pub = hexToBytes(publicKey, 'publicKey', PUBKEY_SIZE);
    const ctx = hexToBytes(contextHash, 'contextHash', CONTEXT_HASH_SIZE);
 ...
 * @throws If inputs are malformed or the WASM call fails.
 */
export async function getConfidentialSendProof(params) {
    ...
    const txBlinding = hexToBytes(params.txBlindingFactor, 'txBlindingFactor', BLINDING_FACTOR_SIZE);
    const ctx = hexToBytes(params.contextHash, 'contextHash', CONTEXT_HASH_SIZE);
    const amountCommitment = hexToBytes(params.amountCommitment, 'amountCommitment', PEDERSEN_COMMIT_SIZE);
    const participants = params.participants.map((participant, index) => rawParticipant(participant, `participants[${index}]`));
