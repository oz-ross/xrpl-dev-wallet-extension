import { BLINDING_FACTOR_SIZE, ELGAMAL_TOTAL_SIZE, PEDERSEN_COMMIT_SIZE, PRIVKEY_SIZE, PUBKEY_SIZE, } from './constants.js';
import { bytesToHex, hexToBytes } from './hex.js';
import { assertUint64, U64_MAX } from './internal.js';
 ...
 */
export async function encryptAmount(amount, publicKey, blindingFactor) {
    assertUint64(amount, 'amount');
    const pub = hexToBytes(publicKey, 'publicKey', PUBKEY_SIZE);
    const blinding = hexToBytes(blindingFactor, 'blindingFactor', BLINDING_FACTOR_SIZE);
    return withModule((mod, marshaller) => {
 ...
 * WASM call fails.
 */
export async function decryptAmount(ciphertext, privateKey, rangeHigh) {
 ...
 * @throws If inputs are malformed or the WASM call fails.
 */
export async function getPedersenCommitment(amount, blindingFactor) {
    assertUint64(amount, 'amount');
    const blinding = hexToBytes(blindingFactor, 'blindingFactor', BLINDING_FACTOR_SIZE);
    return withModule((mod, marshaller) => {
        const blindingPtr = marshaller.allocBytes(blinding);
