import { BLINDING_FACTOR_SIZE, ELGAMAL_TOTAL_SIZE, PEDERSEN_COMMIT_SIZE, PUBKEY_SIZE, } from './constants.js';
 ...
 */
export function assertUint64(value, label, max = U64_MAX) {
...
export function assertUint32(value, label) {
    if (!Number.isInteger(value) || value < 0 || value > U32_MAX) {
        throw new Error(`${label} must be an integer in [0, ${U32_MAX}] (got ${value})`);
    }
}
/**
 ...
 */
export function rawParticipant(participant, label) {
    return {
        publicKey: hexToBytes(participant.publicKey, `${label}.publicKey`, PUBKEY_SIZE),
        ciphertext: hexToBytes(participant.ciphertext, `${label}.ciphertext`, ELGAMAL_TOTAL_SIZE),
    };
}
/**
 ...
 */
export function rawPedersenParams(params, label) {
    ...
    return {
        commitment: hexToBytes(params.commitment, `${label}.commitment`, PEDERSEN_COMMIT_SIZE),
        amount: params.amount,
        ciphertext: hexToBytes(params.ciphertext, `${label}.ciphertext`, ELGAMAL_TOTAL_SIZE),
        blindingFactor: hexToBytes(params.blindingFactor, `${label}.blindingFactor`, BLINDING_FACTOR_SIZE),
    };
}
