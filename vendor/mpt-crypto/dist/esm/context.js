...
import { ACCOUNT_ID_SIZE, CONTEXT_HASH_SIZE, ISSUANCE_ID_SIZE, } from './constants.js';
import { bytesToHex, hexToBytes } from './hex.js';
import { assertUint32 } from './internal.js';
 ...
 */
export async function getConvertContextHash(account, issuance, sequence) {
 ...
 * @throws If inputs are malformed or the WASM call fails.
 */
export async function getConvertBackContextHash(account, issuance, sequence, version) {
    assertUint32(sequence, 'sequence');
 ...
 * @throws If inputs are malformed or the WASM call fails.
 */
export async function getSendContextHash(account, issuance, sequence, destination, version) {
    assertUint32(sequence, 'sequence');
    assertUint32(version, 'version');
    const acc = hexToBytes(account, 'account', ACCOUNT_ID_SIZE);
    const iss = hexToBytes(issuance, 'issuance', ISSUANCE_ID_SIZE);
 ...
 * @throws If inputs are malformed or the WASM call fails.
 */
export async function getClawbackContextHash(account, issuance, sequence, holder) {
    assertUint32(sequence, 'sequence');
    const acc = hexToBytes(account, 'account', ACCOUNT_ID_SIZE);
    const iss = hexToBytes(issuance, 'issuance', ISSUANCE_ID_SIZE);
